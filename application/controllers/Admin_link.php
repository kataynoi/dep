...
class Admin_link extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('Admin_link_model', 'crud');
    ...
    public function index()
    {
        ...
        $this->layout->view('admin_link/index', $data);
    }


    function fetch_admin_link()
    {
    ...
    public function del_admin_link()
    {
        $id = $this->input->post('id');

        $rs = $this->crud->del_admin_link($id);
        if ($rs) {
            $json = '{"success": true}';
        } else {
            $json = '{"success": false}';
    ...
    public function  save_admin_link()
    {
        $data = $this->input->post('items');
        if ($data['action'] == 'insert') {
            $rs = $this->crud->save_admin_link($data,$this->group);
            if ($rs) {
            ...
            $rs = $this->crud->update_admin_link($data,$this->group);
            if ($rs) {
                $json = '{"success": true}';
            } else {
                $json = '{"success": false}';
    ...
    {
        $id = $this->input->post('id');
        $rs = $this->crud->get_admin_link($id);
        $rows = json_encode($rs);
