...
class Admin_vision extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->layout->view('admin_vision/index', $data);
    }
    ...
    public function del_admin_vision()
    {
        $id = $this->input->post('id');

        $rs = $this->crud->del_admin_vision($id);
        if ($rs) {
            $json = '{"success": true}';
        } else {
            $json = '{"success": false}';
    ...
    public function  save_admin_vision()
    {
    ...
    public function  get_admin_vision()
    {
        $id = $this->input->post('id');
        $rs = $this->crud->get_admin_vision($id);
        $rows = json_encode($rs);
