...
class Admin_structure extends CI_Controller
{
    public $user_id;
    public $group;
    ...
    public function __construct()
    {
        ...
        $this->load->model('Admin_structure_model', 'crud');
    ...
    public function index()
    {
        ...
        $this->layout->view('admin_structure/index', $data);
    }


    function fetch_admin_structure()
    {
    ...
    public function del_admin_structure()
    {
        $id = $this->input->post('id');

        $rs = $this->crud->del_admin_structure($id);
        if ($rs) {
            $json = '{"success": true}';
        } else {
            $json = '{"success": false}';
    ...
    public function  save_admin_structure()
    {
        $data = $this->input->post('items');
        if ($data['action'] == 'insert') {
            $rs = $this->crud->save_admin_structure($data,$this->group);
            if ($rs) {
            ...
            $rs = $this->crud->update_admin_structure($data,$this->group);
            if ($rs) {
                $json = '{"success": true}';
            } else {
                $json = '{"success": false}';
    ...
    {
        $id = $this->input->post('id');
        $rs = $this->crud->get_admin_structure($id);
        $rows = json_encode($rs);
